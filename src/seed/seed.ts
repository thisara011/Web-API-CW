...
export async function seedDatabase(client: PoolClient, dataset: SeedDataset): Promise<'seeded' | 'already-seeded'> {
  ...
  await insertBatches(client, 'solar_installations', ['id', 'grid_substation_id', 'meter_id', 'site_label', 'capacity_kw', 'commissioned_date'], dataset.installations.map((item) => [item.id, item.gridSubstationId, item.meterId, item.siteLabel, item.capacityKw, item.commissionedDate]), 'DO NOTHING');
